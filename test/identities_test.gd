...
func run(bc: BCTest) -> void:
	...
	await test_get_child_profiles(bc)
...
func test_attach_peer_profile(bc: BCTest) -> void:
	...
	bc.expect_true(
		status == StatusCodes.OK or status == StatusCodes.BAD_REQUEST,
		"Expected 200 or 400 on attach peer profile, got %d" % status
	)
...
func test_child_parent_flow(bc: BCTest) -> void:
	...
	bc.expect_status_ok(to_parent_resp)
	...
	bc.expect_status_ok(to_child2_resp)
	...
	# Return to parent for subsequent tests
	await bc.bc_wrapper.identity_service.switch_to_parent_profile(parent_level)

func test_get_child_profiles(bc: BCTest) -> void:
	bc.begin_test("test_get_child_profiles")
	var response := await bc.bc_wrapper.identity_service.get_child_profiles(true)
	bc.expect_status_ok(response)
	bc.expect_has_key(response, "data")
